...
function removeFirstOccurrences(str, value) {
  ...
  return ind >= 0
    ? str.slice(0, ind).concat(str.slice(ind + value.length))
    : str;
}
...
function countVowels(str) {
  ...
  return str.split('').filter((item) => vowels.has(item)).length;
}
...
function isPalindrome(str) {
  ...
  const str1 = str
    .split('')
    .map((item) => (isLetter(item) ? item.toLowerCase() : ''))
    .join('');
  const n = str1.length;
  for (let i = 0; i <= Math.floor(n / 2); i += 1) {
    if (str1[i].toLowerCase() !== str1[n - i - 1].toLowerCase()) {
      return false;
    }
  }
  return true;
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (isLetter(str[i])) {
      ...
      const ch = String.fromCharCode(
        aCodeCurr + ((str.charCodeAt(i) - aCodeCurr + 13) % 26)
      );
      res += ch;
    } else {
      res += str[i];
